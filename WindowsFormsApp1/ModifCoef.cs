...
{
    public partial class ModifCoef : Form
    {
        ...
        public ModifCoef(Main parent)
        {
            ...
            if (categories[0].Questions.Count > 0)
            {
                ...
                {
                    id = createGroupControls();
                    buffer = Controls.Find("labelText"+id,true);
                    for (int i = 0; i < 1; i++)
                    {
                        buffer[0].Text += "- ";
                    }
                    buffer[0].Text += ques.Enonce;

                    buffer = Controls.Find("coefNum"+id, true);
                    buffer[0].Text = ques.Coef.ToString();
                    NumericUpDown np = (NumericUpDown)buffer[0];
                    np.DecimalPlaces = 2;
                }
            }
            if (categories[0].Subcategories.Count > 0)
            {
                foreach (var subcat in categories[0].Subcategories)
                {
                    showCat(subcat,1);
                }
            }
        ...
        private void showCat(Categorie cat,int subLevel)
        {
            ...
            {
                foreach (var ques in cat.Questions)
                {
                    id = createGroupControls();
                    buffer = Controls.Find("labelText" + id, true);
        ...
        private void saveData(List<Categorie> source, List<Categorie> dest)
        {
            ...
            {
                dest[i].Coef = source[i].Coef;
                if(source[i].Questions.Count > 0)
                {
                    for (int j = 0; j < source[i].Questions.Count; j++)
                    {
                        dest[i].Questions[j].Coef = source[i].Questions[j].Coef;
                    }
                }
                if(source[i].Subcategories.Count > 0)
                {
                    saveData(source[i].Subcategories, dest[i].Subcategories);
                }
            }
        }
        ...
        private void refreshData(List<Categorie> categories, object sender)
        {
            ...
            if (hasBeenFound == false)
            {
                ...
                {
                    if (cat.Questions.Find(x => x.Enonce == buffer[0].Text) != null)
                    {
                        cat.Questions[cat.Questions.FindIndex(x => x.Enonce == buffer[0].Text)].Coef = (float)np.Value;
                        hasBeenFound = true;
                    }
                }
                ...
                if (hasBeenFound == false)
                {
                    foreach (var cat in categories)
                    {
                        if(cat.Subcategories.Count > 0)
                        {
                            refreshData(cat.Subcategories, sender);
                        }
                    }
                }
            }
